...
try:
    ...
    constructors_df = spark.read.csv(
        HDFS_BASE + "/bronze/constructors/full",
        header=False,
        inferSchema=True
    ).toDF(
    ...
    races_df = spark.read.csv(
        HDFS_BASE + "/bronze/races/full/races_initial.csv",
        header=True,
        inferSchema=True
    )

    results_df = spark.read.csv(
        HDFS_BASE + "/bronze/results/full/results_initial.csv",
        header=True,
        inferSchema=True
    )
    ...
    drivers_cleaned_df.coalesce(1).write.mode("overwrite").parquet(
        HDFS_BASE + "/silver/drivers"
    )

    logger.info("Writing constructors silver")
    constructors_cleaned_df.coalesce(1).write.mode("overwrite").parquet(
        HDFS_BASE + "/silver/constructors"
    )

    logger.info("Writing races silver partitioned by year")
    races_cleaned_df.write.mode("overwrite").partitionBy("year").parquet(
        HDFS_BASE + "/silver/races"
    )

    logger.info("Writing results silver partitioned by year")
    results_with_year_df.write.mode("overwrite").partitionBy("year").parquet(
        HDFS_BASE + "/silver/results"
    )
