...
try:
    ...
    races_inc_df = spark.read.csv(
        HDFS_BASE + "/bronze/races/incremental/races_incremental.csv",
        header=True,
        inferSchema=True
    )

    results_inc_df = spark.read.csv(
        HDFS_BASE + "/bronze/results/incremental/results_incremental.csv",
        header=True,
        inferSchema=True
    )
    ...
    races_main_df = spark.read.parquet(HDFS_BASE + "/silver/races")
    results_main_df = spark.read.parquet(HDFS_BASE + "/silver/results")
    ...
    merged_races_df.write.mode("overwrite").partitionBy("year").parquet(
        HDFS_BASE + "/silver/races_temp"
    )

    logger.info("Writing merged results to temp silver path")
    merged_results_df.write.mode("overwrite").partitionBy("year").parquet(
        HDFS_BASE + "/silver/results_temp"
    )
